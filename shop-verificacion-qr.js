/**
 * shop-verificacion-qr.js
 * Manejo de escaneo QR y actualización de estados de entrega
 */
...
function processQRCode(qrCodeData, deliveryId, expectedQrId) {
    ...
    try {
        ...
        console.log('Expected QR ID:', expectedQrId);
        console.log('Scanned QR ID:', qrData.qr_unique_id);

        // Validar que sea del tipo correcto
        if (qrData.delivery_type !== 'shop_delivery') {
            throw new Error('Este QR no corresponde a una entrega del Shop');
        }

        // Validar que sea el QR correcto
        if (qrData.qr_unique_id !== expectedQrId) {
            Swal.fire({
                icon: 'error',
                title: 'QR Incorrecto',
            ...
            if (data.success) {
                Swal.fire({
                    ...
                    html: `
                        ...
                        <p><strong>Pago liberado:</strong> ${data.payment_released ? 'Sí' : 'No'}</p>
                        ${data.payment_released ? `<p><strong>Monto:</strong> ${data.amount} ${data.currency}</p>` : ''}
                    `,
                    confirmButtonText: 'Entendido'
                }).then(() => {
        ...
        Swal.fire({
            icon: 'error',
            title: 'QR Inválido',
            text: 'El código QR escaneado no tiene el formato correcto.',
            confirmButtonText: 'Entendido'
        });
    }
}
...
function updateDeliveryState(deliveryId, newState) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch('shop-update-delivery-state.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
